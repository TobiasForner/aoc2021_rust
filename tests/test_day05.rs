use aoc2021_rust::day05::{unique_intersections, Edge, Point};
...
fn test_edge_intersections() {
    let horizontal = Edge::Horizontal(Point { x: 3, y: 3 }, Point { x: 6, y: 3 });
    ...
    let diag = Edge::Diagonal(Point { x: 5, y: 5 }, Point { x: 3, y: 3 });
    let res = horizontal.intersection(diag, true).len();
    assert_eq!(res, 1);
    let res = diag.intersection(horizontal, true).len();
    assert_eq!(res, 1);
    ...
    let diag = Edge::Diagonal(Point { x: 1, y: 1 }, Point { x: 3, y: 3 });
    let res = diag.intersection(horizontal, true).len();
    assert_eq!(res, 1);
...
#[test]
fn test_points() {
    let edge = Edge::Diagonal(Point { x: 2, y: 2 }, Point { x: 0, y: 0 });
    let res = edge.points().len();
    assert_eq!(res, 3);
}
